...
import { extractEntries, extractEntriesFromText, bedroomsFromText, SITES } from "../parser.mjs";
...
// protection). Jina AI Reader renders the page in a real browser and returns markdown.
...
// The proxy returns markdown, so parse it with the loose text parser (then fall back to HTML).
async function loadSiteEntries(site) {
  ...
  if (entries.length === 0) {
    const content = await fetchViaReader(site.url);
    entries = extractEntriesFromText(content, site.url, site.detailPattern);
    if (entries.length === 0) {
      try { entries = extractEntries(content, site.url, site.detailPattern); } catch { /* keep [] */ }
    }
  }
  return entries;
}
...
// ---- On-demand "all housing" listing -------------------------------------
// Fetches every active site (with proxy fallback) and returns all schemes + status.
export async function listAllSchemes({ disabledSites = new Set() } = {}) {
  const activeSites = SITES.filter((s) => !disabledSites.has(s.name));
  const result = [];
  for (const site of activeSites) {
    try {
      ...
      result.push({ site: site.name, label: site.label, ok: false, error: e.message, schemes: [] });
    }
  }
  return result;
}
...
export function formatAllSchemes(result) {
  ...
  for (const site of result) {
    if (site.error || !site.ok) {
      blocks.push(`<b>${esc(site.label)}</b>\n⚠️ временно недоступен`);
      continue;
    }
    const open = site.schemes.filter((s) => s.open);
    const closed = site.schemes.length - open.length;
    openTotal += open.length;
    let b = `<b>${esc(site.label)}</b>`;
    if (open.length) {
      b += "\n" + open.map((s) => {
        const dub = /dublin|дублин/i.test(`${s.title} ${s.location}`) ? "📍" : "▫️";
        const loc = s.location ? ` — ${esc(s.location)}` : "";
        return `${dub} <a href="${esc(s.url)}">${esc(s.title)}</a>${loc}`;
      }).join("\n");
    } else {
      b += "\n— открытых нет";
    }
  ...
  const header = openTotal
    ? `🏠 <b>Открыто к подаче: ${openTotal}</b>\n📍 = Дублин`
    : "🏠 Сейчас нет открытых схем для подачи.";
