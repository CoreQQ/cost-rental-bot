...
> **Заметка про Tuath.** Tuath блокирует прямые запросы с облачных серверов (Cloudflare), поэтому
> бот берёт его страницу через рендер-прокси (Jina Reader) и разбирает markdown. Если прокси упрётся
> в лимиты — добавьте бесплатный `JINA_API_KEY` (секрет) для более высоких лимитов. На крайний случай
> у Tuath есть и своя подписка «notify me when applications go live» на tuathhousing.ie.
...
В группе можно в любой момент написать **`/all`** (или `список`) — бот пришлёт список всех
**открытых к подаче** схем по всем сайтам, с пометкой 📍 для Дублина и ссылками. Ответ приходит
на ближайшем запуске (≤15 мин); для мгновенного ответа после команды нажми Actions → watch →
**Run workflow**.
