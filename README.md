...
> **Заметка про Tuath.** Tuath блокирует автоматический доступ с облачных серверов (Cloudflare),
> поэтому по умолчанию он отключён через `DISABLED_SITES=Tuath`. Бот ведёт LDA + Respond, а для Tuath
> используйте их собственную подписку «notify me when applications go live» на tuathhousing.ie.
> Чтобы попробовать снова — уберите `Tuath` из `DISABLED_SITES`.
